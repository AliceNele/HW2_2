...
open_indices = [i for i, x in enumerate(open_delta) if x == min(open_delta)] # 找出裡面最小的開盤價格
close_indices = [i for i, x in enumerate(close_delta) if x == min(close_delta)] # 找出裡面最小的收盤價格
...
#==============================================
for i in range(0,len(target)):
    print(" %d 做多損益：%f"  %(bullish_target ,target[i]['bullish_target_close_price'] -   \
                                               target[i]['bullish_target_open_price'] )/    \
                                               target[i]['bullish_target_open_price'])
    
    print(" %d 做空損益：%f" %(bearish_target,target[i]['bearish_target_open_price'] -   \
                                               target[i]['bearish_target_close_price'] )/    \
                                               target[i]['bearish_target_open_price'])