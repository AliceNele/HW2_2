...
def get_data_path(category, filename = ""):
    if filename == "":
        ...
        return "./info/" + category + "/" + filename + ".pkl"

def check_data_exist(category, filename):
    return (filename + ".pkl") in os.listdir(get_data_path(category))

def create_credit_user(): # 創信用卡帳戶
    name = input("請輸入姓名: ")
    idnumber = input("請輸入身分證字號: ")
    if check_data_exist("credit", idnumber): # 先用身分證字號檢查是否已有可用帳戶
        print("已有可用帳戶")
...
def create_bank_user(): # 創銀行帳戶
    name = input("請輸入姓名: ")
    idnumber = input("請輸入身分證字號: ")
    if check_data_exist("bank", idnumber):#先用身分證字號檢查是否已有可用帳戶
        print("已有可用帳戶")
    else: #沒有可用帳戶，繼續輸入密碼
...
def pay_creditcard_fee():
    idnumber = input("請輸入身分證字號: ")
    #檢查是否有銀行跟信用卡帳戶
    if check_data_exist("bank", idnumber) and check_data_exist("credit", idnumber):
        password = input("儲蓄存款帳戶密碼: ")
        with open(get_data_path("bank", idnumber),'rb') as info: #開啟資料
            user = pickle.load(info)
        if password != user.password:#檢查密碼是否正確，情況3
            print("密碼錯誤")
        else: #密碼對了
            print("密碼答對")
            with open(get_data_path("credit", idnumber),'rb') as info: #開啟信用卡資料
            ...
            if user.inmoney >= account.used:  #儲蓄帳戶餘額 > 信用卡費，情況4
               ...
               #with open('./info/bank/'+idnumber+'.pkl','wb') as info:
                ...
                print( "餘額不足")
    #有銀行但沒有信用卡帳戶，情況2
    elif check_data_exist("bank", idnumber) and not check_data_exist("credit", idnumber):
        print( "無須繳費")
    #沒有銀行但有信用卡帳戶，情況1
    elif not check_data_exist("bank", idnumber) and check_data_exist("credit", idnumber):
        print("請先開立帳戶")
        create_bank_user()
